...
void setup() {
 ...
 //tflModel = tflite::GetModel(model);
  ...
  // Check if the model schema version is compatible
 /*if (tflModel->version() != TFLITE_SCHEMA_VERSION) {
   // if (tflModel->version() != TFLITE_CHECK_LE) {
...
void loop() {
  ...
  while (samplesRead < numSamples) {
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      // increment the sample read count
      samplesRead++;
    }
  }

  // (add your TensorFlow Lite model inference code here)
}
*/
//################################FINAL#############################
  ...
  while (samplesRead < numSamples) {
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (samplesRead == numSamples) {
        ...
        if (GESTURES[maxConfidence] == "1") {
            digitalWrite(2, HIGH);
            digitalWrite(3, LOW);
            digitalWrite(4, LOW);
        } else if (GESTURES[maxConfidence] == "2") {
            digitalWrite(2, LOW);
            digitalWrite(3, HIGH);
            digitalWrite(4, LOW);
        } else if (GESTURES[maxConfidence] == "3") {
            digitalWrite(2, LOW);
            digitalWrite(3, LOW);
            digitalWrite(4, HIGH);
        } else {
